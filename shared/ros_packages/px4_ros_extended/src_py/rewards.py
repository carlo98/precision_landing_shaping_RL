...
class Reward:
    ...
    def get_reward(self, obs, norm_obs, action, landed, ir_beacon_position, eps_pos_xy, eps_vel_xy):
        done = False
        c = 0.0
        ...
        if landed and np.abs(obs[3]) <= eps_vel_xy and np.abs(obs[4]) <= eps_vel_xy \
                and np.sqrt(ir_beacon_position[0]**2 - obs[0]**2) <= eps_pos_xy and np.sqrt(ir_beacon_position[1]**2 - obs[1]**2) <= eps_pos_xy:
        ...
        elif landed and (np.sqrt(ir_beacon_position[0]**2 - obs[0]**2) > eps_pos_xy or np.sqrt(ir_beacon_position[1]**2 - obs[1]**2) > eps_pos_xy):
            print("Landed in wrong place.")
            done = True
            
        # Landed in obj
        elif landed and np.sqrt(ir_beacon_position[0]**2 - obs[0]**2) <= eps_pos_xy and np.sqrt(ir_beacon_position[1]**2 - obs[1]**2) <= eps_pos_xy:
